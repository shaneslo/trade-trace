...
## 2026-06-23 - Accessible tooltips and ARIA positioning
**Learning:** Found a pattern where `aria-label` attributes were placed on inner `<span>` wrappers (like the `+` sign for "Add node") instead of the interactive `<Button>` element itself. Also, icon-only buttons like the Theme toggle were using inner `<span className="sr-only">` instead of putting `aria-label` on the button.
**Action:** When working with icon-only buttons, ensure the `<Button>` itself has the `aria-label` and `title` attributes. Inner text or icon spans should use `aria-hidden="true"` to prevent screen readers from redundantly announcing visual symbols (like "plus").
